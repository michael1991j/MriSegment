...
int main(int argc, char **argv) {
	...
	char* No_Fat_Path =
			fileconfig->GetSettings("FindBoneFemerTrans", "No_Fat_Path",
					"/home/mri/Dropbox/School/MRI Segmentation/SampleData/SaikatKnee2012/002-SagittalCube-NoFatSat/");
	char* Fat_Path =
			fileconfig->GetSettings("FindBoneFemerTrans", "Fat_Path",
	...
	for (int i = 0; i < fat->Data->Transversal->size(); i++) {
		...
		FindBoneFemurTrans * process = new FindBoneFemurTrans(&Imagesets,
				&results, i, config);
		process->Setup();
		process->Preprocess();
		process->Segment();
